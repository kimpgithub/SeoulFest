...
fun SeoulFilter(navController: NavHostController, initialSelectedDistricts: List<String>) {
    ...
    Column(
        ...
        SeoulFilterTopBar(navController)
        SeoulDistrictGrid(selectedDistricts) { updatedDistricts ->
            selectedDistricts = updatedDistricts.toMutableList()
        }
        Spacer(modifier = Modifier.weight(1f))
        ConfirmButton(selectedDistricts, navController)
    }
}

@OptIn(ExperimentalMaterial3Api::class)
@Composable
fun SeoulFilterTopBar(navController: NavHostController) {
    TopAppBar(
        title = { Text("서울특별시") },
        actions = {
...
@Composable
fun SeoulDistrictGrid(
    selectedDistricts: List<String>,
    onDistrictSelected: (List<String>) -> Unit
) {
...
@Composable
fun ConfirmButton(selectedDistricts: List<String>, navController: NavHostController) {
    Button(
        onClick = {
